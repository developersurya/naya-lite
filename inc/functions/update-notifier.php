...
function update_notifier_menu() {
	...
	if(version_compare($theme_data['Version'], $xml->latest, '<')) {
            add_dashboard_page($theme_data['Name'] . ' - Theme Updates', $theme_data['Name'] . '<span class="update-plugins count-1"><span class="update-count">'.$xml->latest.'</span></span>', 'read', sanitize_title($theme_data['Name']) . '-updates', 'update_notifier');
	}
}
...
function get_latest_theme_version($interval) {
	...
	$notifier_file_url = 'http://ldsclient.com/notifier.xml';

	$db_cache_field = 'sampression-notifier-cache';
	$db_cache_field_last_updated = 'sampression-notifier-last-updated';
